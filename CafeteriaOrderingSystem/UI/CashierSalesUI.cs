...
{
    public partial class CashierSalesUI : Form
    {
        public CashierSalesUI(int SaleID, int userID)
        {
            InitializeComponent();
            this.SaleID = SaleID;
            this.userID = userID;
        ...
        private void CashierSalesUI_Load(object sender, EventArgs e)
        {
            label5.Text = CalculateTotalBill(dataGridView1).ToString() + ": الارباح";
            label7.Text = CalculateTotalBillEx(dataGridView2).ToString() + ": المنصرفات";
            ...
            foreach (DataGridViewRow Row in  dataGridView1.Rows)
                    {
                        try
                        {
                            ...
                            {
                                string ProductName = (string)Row.Cells["Column3"].Value;
                                ...
                                ProductDetail.Add(new SaleItemsBLL() { ProductName = ProductName, ProductPrice = ProductPrice, Quantity = Quantity, ProductTotal = ProductTotal, UserID = userID, PIDs = ProductID });
                            }
                        }
                        finally
                        {
